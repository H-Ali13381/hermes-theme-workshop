...
from ..config import MODEL, PALETTE_SLOTS, DESIGN_REQUIRED_KEYS
...
for a desktop rice. Build on the creative direction already established.
...
- kvantum_theme: e.g. "catppuccin-mocha-teal"
- cursor_theme: e.g. "catppuccin-macchiato-teal-cursors"
...
4. When confirmed, output {sentinel} followed by the final JSON on the next line
...
```
""".format(sentinel=DESIGN_SENTINEL)
...
def refine_node(state: RiceSessionState) -> dict:
    ...
    if DESIGN_SENTINEL in response.content:
        design = _extract_design_json(response.content)
        if design:
            ...
            return {
                "messages": messages + [AIMessage(content=response.content.split(DESIGN_SENTINEL)[0].strip())],
                "design": design,
                "current_step": 3,
            }
...
def _extract_design_json(content: str) -> dict | None:
    ...
    if match:
        try:
            ...
            return obj if _validate_design(obj) else None
        except Exception:
            pass
    return None


def _validate_design(d: dict) -> bool:
