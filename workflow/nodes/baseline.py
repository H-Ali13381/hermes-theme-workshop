...
def baseline_node(state: RiceSessionState) -> dict:
    ...
    if not audit_script.exists():
        ...
        return {"baseline_ts": ts, "current_step": 4}

    result = subprocess.run(
        [sys.executable, str(audit_script), "--capture"],
        capture_output=True, text=True, timeout=30,
    )

    if result.returncode != 0:
        print(f"  [WARN] Baseline capture exited {result.returncode}: {result.stderr[:200]}")
        return {"baseline_ts": ts, "current_step": 4, "errors": [f"baseline_warn: {result.stderr[:200]}"]}
